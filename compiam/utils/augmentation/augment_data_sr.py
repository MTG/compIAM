...
def apply_nmf_separation(x, fs, hopSize, nFFT, W):
    ...
    X = librosa.stft(x, n_fft=nFFT, hop_length=hopSize, win_length=paramSTFT['blockSize'], window='hann', center=True, pad_mode='reflect')
...
def augment_data_stroke_remix(audio_path, out_dir, fs, gain_factors, nmf_templates_path, winDur, hopDur):
    # analysis parameters
    ...
    nmf_templates = np.array([item for key in ['rb', 'rt', 'd'] for item in nmf_templates[key].T]).T
    ...
    x_resyn = gain_factors[0]*comp_rb + gain_factors[1]*comp_rt + gain_factors[2]*comp_d
    x_resyn /= np.max(np.abs(x_resyn))

    audio_save_name = PurePath(audio_path).name.replace('.wav', '_sr_bass_%2.2f_treble_%2.2f_damp_%2.2f.wav'%tuple(gain_factors))
...
if __name__=='__main__':
    ...
    parser.add_argument("--params", nargs='+', type=tuple_list, default=[(0.6, 2, 0.2), (0.6, 2, 3), (2, 0.5, 0.2), (2, 0.5, 3)], help="list of 3-tuples with gain factors for remixing. Tuple entries correspond to each of bass, treble, & damped components.")
    parser.add_argument("--templates", type=str, default="./nmf_templates.npy", help="path to saved nmf templates")
    ...
    _ = Parallel(n_jobs=args.n_jobs)(delayed(augment_data_stroke_remix)(args.input, args.output, args.fs, gain_set, args.templates, winDur, hopDur)
