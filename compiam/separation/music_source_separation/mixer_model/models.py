...
class ConvTDFNet(nn.Module):
    ...
    def stft(self, audio_norm):
        ...
        for i in range(factor):
            audio_chunk = audio_norm[:, :, self.chunk_size*i : self.chunk_size*(i+1)].float()
            window = torch.hann_window(window_length=self.n_fft, periodic=True,dtype=torch.float, device=audio_norm.device)
            audio_chunk = audio_chunk.reshape([-1, self.chunk_size])
            spec = torch.stft(audio_chunk.float(), n_fft=self.n_fft, hop_length=self.hop_length, window=window, center=True, return_complex = True)
    ...
    def istft(self, spec):
        ...
        for i in range(factor):
            ...
            freq_pad = torch.zeros([1, self.dim_c, self.n_bins - self.dim_f, self.dim_t], device=spec.device).float()
            window = torch.hann_window(window_length=self.n_fft, periodic=True, dtype=torch.float, device=spec.device)
            spec_chunk = torch.cat([spec_chunk, freq_pad.repeat([spec_chunk.shape[0], 1, 1, 1])], -2)  
            
            spec_chunk = spec_chunk.permute([0, 2, 3, 1]).contiguous() 
            spec_chunk = torch.view_as_complex(spec_chunk)
            spec_chunk = torch.istft(spec_chunk, n_fft=self.n_fft, hop_length=self.hop_length, window=window, center=True, return_complex = False)
    ...
    def forward(self, audio,  iteration=0):
        ...
        x = x.transpose(-1, -2)
        ds_outputs = []
        for i, (encoding_block, ds_block) in enumerate(zip(self.encoding_blocks, self.ds)):
            x = encoding_block(x)
            ds_outputs.append(x)
            x = ds_block(x)
        x = self.bottleneck_block(x)

        for i, (decoding_block, us_block) in enumerate(zip(self.decoding_blocks, self.us)):
            x = us_block(x)
            x = x * ds_outputs[-i - 1]
            x = decoding_block(x)
