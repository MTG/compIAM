...
def extract_features(audio_dir, annotations_dir, save_dir, mode):
    ...
    if mode == "voc":
        input_hops_stm = {
            1.: int(np.floor(1./(hopsize/fs))),
            2.: int(np.floor(0.5/(hopsize/fs))),
            4.: int(np.floor(0.5/(hopsize/fs))),
            8.: int(np.floor(0.1/(hopsize/fs)))}
        aug_ts_versions = {
            1.: [0.8,0.92,1.0,1.12],
            2.: [0.8,0.92,1.0,1.12],
            4.: [0.8,0.92,1.0,1.12],
            8.: [0.8,0.84,0.88,0.92,0.96,1.0,1.04,1.08,1.12,1.16]}

    elif mode == "pakh":
        input_hops_stm = {
            1.: int(np.floor(0.5/(hopsize/fs))),
            2.: int(np.floor(0.5/(hopsize/fs))),
            4.: int(np.floor(1./(hopsize/fs))),
            8.: int(np.floor(1./(hopsize/fs))),
            16.: int(np.floor(0.5/(hopsize/fs)))}
        aug_ts_versions = {
            1.: [0.8,0.84,0.92,0.96,1.0,1.04,1.12,1.16],
            2.: [0.8,0.84,0.92,0.96,1.0,1.04,1.12,1.16],
            4.: [0.8,0.92,1.0,1.12],
            8.: [0.8,0.92,1.0,1.12],
            16.: [0.8,0.84,0.92,0.96,1.0,1.04,1.12,1.16]}
    ...
    #main
    annotations = np.loadtxt(os.path.join(annotations_dir, "section_boundaries_labels.csv", delimiter=",", dtype=str))
    ...
    for i, item in enumerate(songlist):
        ...
        try: 
            ...
            aug_ts = 1.0
        if (label_stm not in [1.,2.,4.,8.,16.]):
            continue
        ...
        melgram = librosa.feature.melspectrogram(
            x, sr=fs, n_fft=nfft, hop_length=hopsize, win_length=winsize, n_mels=40, fmin=20, fmax=8000)
