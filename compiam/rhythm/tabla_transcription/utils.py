		...
		with h5.File(train_val_data_filepaths['train'], 'a') as hf_fold:
			for fold in folds['train']:
				if 'data' not in hf_fold:
					hf_fold.create_dataset('data', data=hf_data[f'fold{fold}/data'], maxshape=(None,None))
				else:   
					hf_fold['data'].resize((hf_fold['data'].shape[0] + hf_data[f'fold{fold}/data'].shape[0]), axis = 0)
					hf_fold['data'][-hf_data[f'fold{fold}/data'].shape[0]:] = hf_data[f'fold{fold}/data']
	return
