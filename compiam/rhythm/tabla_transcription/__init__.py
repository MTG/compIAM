...
class FourWayTabla:
    ...
    def train_epoch(
        self, model, training_generator, criterion=torch.nn.BCELoss(reduction="none")
    ):
        model.train()
        n_batch = 0
        loss_epoch = 0
        for local_batch, local_labels, local_weights in training_generator:
            ...
            local_batch, local_labels, local_weights = (
                local_batch.double().to(device),
                local_labels.double().to(device),
                local_weights.double().to(device),
            )
        ...
        for ci, c in enumerate(categories):
            ...
            for epoch in range(max_epochs):
                ...
                model, loss_epoch = self.train_epoch(
                    model, training_generator, criterion
                )
                ...
                if val_loss_epoch[-1] == min(val_loss_epoch):
                    if model_save_dir:
                        torch.save(
                            model.state_dict(),
                            os.path.join(model_save_dir, f"saved_model_{args.fold}.pt"),
                        )
            ...
            if plot_save_dir:
                ...
                plot_save_filepath = os.path.join(
                    plot_save_dir, f"loss_curves_{args.fold}.png"
                )
    ...
    def get_generators(
        ...
        train_val_data_filepaths = {
            ...
            "validation": os.path.join(
                filepath, f"labels_weights_val_{aug_method}.hdf5"
            ),
        }
        ...
        labels_weights_orig_filepath = os.path.join(
            filepath, f"labels_weights_orig_{category}.hdf5"
        )
        labels_weights_aug_filepath = os.path.join(
            filepath, f"labels_weights_{aug_method}_{category}.hdf5"
        )
        make_train_val_split(
            folds,
        ...
        songlist_orig = np.loadtxt(
            os.path.join(filepath, "songlists/songlist_orig.txt"), dtype=str
        )
        songlist_aug = np.loadtxt(
            os.path.join(filepath, f"/songlists/songlist_{aug_method}.txt"), dtype=str
        )
