    ...
    def train(
        ...
        for ci,c in enumerate(categories):
            ...
            for epoch in range(max_epochs):
                ...
                ##validation
                model, loss_epoch = self.validate(model, criterion, validation_generator)
                val_loss_epoch.append(loss_epoch)

                #print loss in current epoch
                logger.info(f'Epoch no: {epoch}/{max_epochs}\tTrain loss: {train_loss_epoch[-1]}\tVal loss: {val_loss_epoch[-1]}')
