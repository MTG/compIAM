...
class AksharaPulseTracker:
    ...
    """
    def __init__(self, Nfft=4096, frmSize=1024, Fs=44100, hop=512, 
    ...
    def extract(self, fname, verbose=True):
        ...
        # Get onset functions
        onsFns = self.getOnsetFunctions(fname, self.Nfft, self.frmSize, self.Fs, 
                                    self.fTicks, self.hop, self.numBands, self.fBands, verbose)
        onsFn = onsFns[:, 6].copy()
        onsTs = onsFns[:, 0].copy()
        ...
        # Construct tempogram
        TG, TCts, BPM = tempogram_viaDFT(onsFn.copy(), self.tempoWindow, self.featureRate, self.stepSize, self.BPM, verbose)
        TG = np.abs(normalizeFeature(TG, 2))

        # Estimate the tempo curve - IAI curve
        TCRaw = getTempoCurve(TG.copy(), BPM, self.minBPM, self.octTol, self.theta, self.delta, verbose)
        TCperRaw = 60.0 / TCRaw

        # Estimate akshara/matra period
        mmpFromTC = self.getMatraPeriodEstimateFromTC(TCperRaw, self.Nbins, self.minBPM, self.wtolHistAv, verbose)
        TCper, TCcorrFlag = correctOctaveErrors(TCperRaw, mmpFromTC, self.octCorrectParam, verbose)
        TC = 60.0 / TCper

        # Candidate estimation
        akCandLocs, akCandTs, akCandWts, akCandTransMat = self.estimateAksharaCandidates(onsTs, onsFn.copy(), TCper, TCts,
                                                                                         mmpFromTC, self.pwtol, self.thres, 
                                                                                         self.ignoreTooClose, self.decayCoeff, verbose)
        Locs = akCandLocs
        ts = akCandTs
        Wts = akCandWts
        ...
        # Candidate selection
        aksharaLocs, aksharaTimes = self.DPSearch(TransMatCausal, ts, pers, Locs, Wts, self.backSearch, self.alphaDP, verbose)
    ...
    def getOnsetFunctions(self, fname, Nfft, frmSize, Fs, fTicks, hop, numBands, fBands, verbose=True):
    ...
    def getMatraPeriodEstimateFromTC(self, TCper, Nbins, minBPM, wtolHistAv, verbose=True):
        if verbose:
    ...
    def estimateAksharaCandidates(self, tstamps, onsFn, TCper, TCts, medIAI, pwtol, thres, ignoreTooClose, decayCoeff, verbose=True):
        if verbose:
    ...
    def DPSearch(self, TransMatCausal, ts, pers, Locs, Wts, backSearch, alphaDP, verbose=True):
        if verbose:
