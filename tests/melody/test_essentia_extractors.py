...
def _predict_normalized_pitch():
    ...
    assert np.all(np.isclose(pitch[:10, 0], np.array([0., 0.00290249, 0.00580499, 0.00870748, 0.01160998, 0.01451247, 0.01741497, 0.02031746, 0.02321995, 0.02612245])))
    assert np.all(np.isclose(pitch[140:150, 1], np.array([274.00152588, 270.85430908, 269.29431152, 267.74328613, 266.20120239, 263.14358521, 261.62796021, 260.12109375, 258.6229248, 257.13336182])))
