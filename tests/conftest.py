...
def pytest_addoption(parser):
    for option in OPTIONS:
        parser.addoption("--" + option, action="store_true", default=False, \
...
def pytest_configure(config):
    for option in OPTIONS:
        config.addinivalue_line("markers", option + ": run optional " + 
            option.replace("_", "and") + " tests")
    ...
    # One option already added. Warning user...
        ...
        if not config.getoption("--" + option):
            ...
            print("\n\nIMPORTANT: You have entered two testing markers. " +
                ...
                "in the terminal. Running tests only for: " + running_test + "\n")
            skip_option = pytest.mark.skip(reason="need --" + option + " option to run")
            for item in items:
                if option in item.keywords:
                    item.add_marker(skip_option)
